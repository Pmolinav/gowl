...
class PlayerBetSelectionControllerTest extends BaseUnitTest {
    ...
    private void givenValidDTO() {
        dto = new PlayerBetSelectionDTO(1L, 2L, 3L, BigDecimal.ONE);
    }

    private void whenFindAllReturnsList() {
        expectedList = List.of(
                new PlayerBetSelectionDTO(1L, 2L, 3L, BigDecimal.ONE),
                new PlayerBetSelectionDTO(1L, 2L, 4L, BigDecimal.ONE)
        );
        when(playerBetSelectionServiceMock.findAll()).thenReturn(expectedList);
    }
    ...
    private void whenFindAllThrowsServerError() {
        when(playerBetSelectionServiceMock.findAll()).thenThrow(new CustomStatusException("Error", 500));
    }
    ...
    private void whenFindByIdThrowsServerError() {
        when(playerBetSelectionServiceMock.findById(1L)).thenThrow(new CustomStatusException("Error", 500));
    }

    private void whenCreateReturnsId() {
        when(playerBetSelectionServiceMock.create(any())).thenReturn(1L);
    }

    private void whenCreateThrowsServerError() {
        when(playerBetSelectionServiceMock.create(any())).thenThrow(new CustomStatusException("Error", 500));
    }

    private void whenDeleteDoesNothing() {
        doNothing().when(playerBetSelectionServiceMock).delete(1L);
    }

    private void whenDeleteThrowsNotFound() {
        doThrow(new NotFoundException("Not found")).when(playerBetSelectionServiceMock).delete(1L);
    }

    private void whenDeleteThrowsServerError() {
        doThrow(new CustomStatusException("Error", 500)).when(playerBetSelectionServiceMock).delete(1L);
    }

    // --- CALL CONTROLLER ---

    private void andFindAllIsCalled() {
        result = playerBetSelectionController.findAllSelections(requestUid);
    }

    private void andFindByIdIsCalled() {
        result = playerBetSelectionController.findSelectionById(requestUid, 1L);
    }

    private void andCreateIsCalled() {
        BindingResult bindingResult = mock(BindingResult.class);
        when(bindingResult.hasErrors()).thenReturn(false);
        result = playerBetSelectionController.createSelection(requestUid, dto, bindingResult);
    }
    ...
    private void andDeleteIsCalled() {
        result = playerBetSelectionController.deleteSelection(requestUid, 1L);
    }

    // --- VERIFY ---

    private void thenVerifyFindAllCalled() {
        verify(playerBetSelectionServiceMock, times(1)).findAll();
    }

    private void thenVerifyFindByIdCalled() {
        verify(playerBetSelectionServiceMock, times(1)).findById(1L);
    }

    private void thenVerifyCreateCalled() {
        verify(playerBetSelectionServiceMock, times(1)).create(any());
    }
